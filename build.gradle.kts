...
val pluginMain = properties("plugin.main")
...
//    compileOnly(files(FILES))
    ANNO.forEach(::annotationProcessor)
    IMPL.forEach(::implementation)
    COMP_ONLY.forEach(::compileOnly)
    SHADOW.forEach(::shadow)

    // Other plugins
    OTHER_PLUGINS.forEach(::compileOnly)
}
...
tasks.processResources {
    ...
    doFirst {
        println("Version: ${project.version}, Name: ${project.name}, Main: $pluginMain")
    }

    inputs.property("name", project.name)
    inputs.property("version", project.version)
    inputs.property("main", pluginMain)
